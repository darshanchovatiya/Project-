...
def rating(request):
    if request.method == 'POST':
        ...
        dr=item.objects.get(p_id = request.session['rat'])
        rate = dr.rating
        pid = dr.p_id
        frat = float(rat) + float(rate)
        lrate = frat/2
        request.session['lrat']=lrate
        item.objects.filter(p_id = pid).update(rating=lrate)
...
def csbill(request,id):
    ...
    elif server.objects.filter(s_id = id).exists():
